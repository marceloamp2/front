...
const getUsers = async (page) => {
  try {
    ...
    if (page) {
      url = `${api.baseUrl}/api/users?paginate=true&page=${page}`
    } else if (filters.value.search) {
      url = `${api.baseUrl}/api/users?paginate=true&filters[search]=${filters.value.search}`
      ...
      url = `${api.baseUrl}/api/users?paginate=true&${queryString}`
    } else {
      url = `${api.baseUrl}/api/users?paginate=true`
    }
...
const getRoles = async () => {
  try {
    ...
    const url = `${api.baseUrl}/api/roles?paginate=false`
...
const openCreateUserModal = () => {
  ...
  user.value.password_confirmation = '12345678'
  user.value.status = 'active'
  user.value.role_id = ''
  errors.value = {}
  modalTitle.value = 'Incluir usuário'
...
const formValidate = () => {
  ...
  if (!user.value.role_id) {
    errors.value.role_id = 'O cargo é obrigatório'
  }
  ...
  try {
    ...
    if (formValidate()) {
      ...
      if (user.value.id) {
        ...
        const url = `${api.baseUrl}/api/users/${user.value.id}`

        response = await axios.put(url, user.value, {
          headers: {
            Accept: 'application/json',
            Authorization: `Bearer ${authStore.token}`
          }
        ...
        response = await axios.post(url, user.value, {
          headers: {
            Accept: 'application/json',
            Authorization: `Bearer ${authStore.token}`
          }
      ...
      await getUsers(users.value.current_page)
    }
...
<template>
  ...
  <h1 class="py-3 text-sm font-medium">Usuários</h1>
  ...
  <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
      ...
      >
        ...
        >
          <tr>
            ...
            <th class="px-4 py-3" scope="col">Cargo</th>
        ...
        <tbody>
          ...
          >
            ...
            <td v-if="roles.length > 0" class="px-4 py-3">{{ getRoleName(user.role_id) }}</td>
            <td class="px-4 py-3">
              <span
                :class="[
                  user.status === 'active'
            ...
            <td class="px-4 py-3 flex items-center justify-end">
              <button class="text-blue-700 mr-3" type="button" @click="openEditUserModal(user)">
                <svg
                  class="w-4 h-4"
                  fill="currentColor"
                  viewBox="0 0 512 512"
              ...
              <button class="text-red-700" type="button" @click="openDeleteUserModal(user)">
                <svg
                  class="w-4 h-4"
                  fill="currentColor"
                  viewBox="0 0 448 512"
    ...
    <Pagination v-if="!loadingStore.loading" :registers="users" @go-to-page="getUsers" />
  </div>
  <!--    End table-->
          ...
          <form>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-2">
              ...
              <div class="mb-5">
                ...
                <p v-if="errors.role_id" class="mt-2 text-sm text-red-600 dark:text-red-500">
                  {{ errors.role_id }}
                </p>
              </div>
            </div>
