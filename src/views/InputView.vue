...
const getInputs = async (page) => {
  try {
    ...
    if (page) {
      url = `${api.baseUrl}/api/inputs?paginate=true&page=${page}`
    } else if (filters.value.search) {
      url = `${api.baseUrl}/api/inputs?paginate=true&filters[search]=${filters.value.search}`
    } else {
      url = `${api.baseUrl}/api/inputs?paginate=true`
    }
...
const openCreateInputModal = () => {
  ...
  input.value.name = ''
  errors.value = {}
  modalTitle.value = 'Incluir insumo'
  createEditModal.value.show()
}

const openEditInputModal = (data) => {
  input.value.id = data.id
...
const storeUpdateInput = async () => {
  try {
    ...
    if (formValidate()) {
      ...
      if (input.value.id) {
        const url = `${api.baseUrl}/api/inputs/${input.value.id}`

        response = await axios.put(url, input.value, {
          headers: {
            Accept: 'application/json',
            Authorization: `Bearer ${authStore.token}`
          }
        ...
        response = await axios.post(url, input.value, {
          headers: {
            Accept: 'application/json',
            Authorization: `Bearer ${authStore.token}`
          }
      ...
      await getInputs(inputs.value.current_page)
    }
...
const openDeleteInputModal = (data) => {
  input.value.id = data.id
...
<template>
  ...
  <h1 class="py-3 text-sm font-medium">Insumos</h1>
  ...
  <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
    ...
    <div v-if="!loadingStore.loading" class="overflow-x-auto">
      <table
        v-if="inputs.data && inputs.data.length > 0"
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-3 flex items-center justify-end">
              <button class="text-blue-700 mr-3" type="button" @click="openEditInputModal(input)">
                <svg
                  class="w-4 h-4"
                  fill="currentColor"
                  viewBox="0 0 512 512"
              ...
              <button class="text-red-700" type="button" @click="openDeleteInputModal(input)">
                <svg
                  class="w-4 h-4"
                  fill="currentColor"
                  viewBox="0 0 448 512"
    ...
    <Pagination v-if="!loadingStore.loading" :registers="inputs" @go-to-page="getInputs" />
  </div>
  <!--  End table-->
